from radx_reporter.studies import meta_parser
from radx_reporter.studies import vocabulary
from radx_reporter.studies import study
import datetime
import pytz
import pandas as pd
import pytest
...
class TestMetaParser:
    ...
    def example_dataframe(self):
        data = {
            ...
            'description': ['There are two device studies under the 2183 banner. The 2183b study uses an enhanced visual display compared to the original study (2183a), so it has been renamed. These prospective clinical studies seek to examine the performance of the 2183 device (a and b), a lateral flow immunoassay for the point-of-care (POC) detection of SARS-CoV-2 nucleocapsid protein antigen, compared to the Roche 6800 Cobas PCR for SARS-CoV-2 assay. The goal of the 2183b study is for EUA approval for the device to be used among symptomatic persons by trained healthcare personnel collected mid-turbinate specimens. Note: no device performance data will be included in the data files.', None],
            'estimated_participants': [488, None],
            'institutes_supporting_study - CODED': ['NHLBI; NIBIB', None],
            'phs': ['phs002682', "failure"],
            'source - CODED': ['Questionnaire/Survey; Smartphone; COVID Testing Device', None],
    ...
    def test_parse_institute(self, example_dataframe):
        ...
        assert set(institutes) == {vocabulary.NihInstitute.NHLBI, vocabulary.NihInstitute.NIBIB}
    ...
    def test_parse_population(self, example_dataframe):
        population, population_range = meta_parser.parse_population(example_dataframe.loc[0])
    ...
    def test_parse_population_unknown(self, example_dataframe):
        population, population_range = meta_parser.parse_population(example_dataframe.loc[1])
    ...
    def test_parse_data_types(self, example_dataframe):
        ...
        assert set(data_types) == {vocabulary.DataType.QUESTIONNAIRE, vocabulary.DataType.CLINICAL}
    ...
    def test_parse_dates(self, example_dataframe):
        ...
        assert start_date == datetime.datetime(2020, 10, 16, 15, 32, 30, tzinfo=pytz.UTC)
