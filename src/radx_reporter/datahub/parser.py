...
ref_institutes = list(NihInstitute)
ref_study_designs = list(StudyDesign)
ref_population_ranges = list(PopulationRange)
ref_data_types = list(DataType)
ref_study_domains = list(StudyDomain)
ref_collection_methods = list(CollectionMethod)
...
def parse_nih_institutes(row):
    ...
    if pd.isna(nih_institute_text):
        ...
        nih_institutes = [institute for institute in ref_institutes if institute.label in nih_institute_text]
...
def parse_collection_methods(row):
    ...
    if pd.isna(collection_method_text):
        ...
        collection_methods = [method for method in ref_collection_methods if method.label in collection_method_text]
...
def parse_study_designs(row):
    ...
    if pd.isna(study_design_text):
        ...
        study_designs = [design for design in ref_study_designs if design.label in study_design_text]
...
def parse_population(row):
    ...
    if pd.isna(population_text):
        ...
        if match:
            ...
            population_range = [pop_range for pop_range in ref_population_ranges if pop_range.lower_bound <= population <= pop_range.upper_bound].pop()
        else:
            population = None
            population_range = PopulationRange.MISSING
    else:
        population = int(population_text)
        population_range = [pop_range for pop_range in ref_population_ranges if pop_range.lower_bound <= population_text <= pop_range.upper_bound].pop()
...
def parse_data_types(row):
    ...
    if pd.isna(data_type_text):
        ...
        data_types = [data_type for data_type in ref_data_types if data_type.label in data_type_text]
        if len(data_types) == 0:
...
def parse_study_domains(row):
    ...
    study_domains = [topic for topic in ref_study_domains if topic.label.lower() in study_topic_text]
...
def parse_studies_from_file(file_name: str, sheet_name: str):
